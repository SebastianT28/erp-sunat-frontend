...
export default function Seccion1({ alSiguiente, datos, actualizarDatos }: { alSiguiente: () => void, datos: any, actualizarDatos: (d: any) => void }) {
        ...
        <div className="bg-white p-6 border-b flex justify-around items-center text-center">
          {[
            ...
            <div key={i} className="flex flex-col items-center flex-1 relative">
              ...
              <span className={`text-xs md:text-sm font-medium ${step.active ? 'text-[#0071BC]' : 'text-gray-400'}`}>{step.t}</span>
        ...
        <div className="p-6 md:p-10">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
            <div className="space-y-6">
              <div className="relative group">
                <label className="flex items-center text-gray-700 font-bold mb-2">
                  ...
                  <div className="absolute left-40 bottom-full mb-2 hidden group-hover:block w-48 p-2 bg-gray-800 text-white text-[10px] rounded shadow-lg z-50">Ingrese el mes y año (ejemplo: 03/2026).</div>
                </label>
                <input type="text" value={datos.periodoTributario} onChange={e => actualizarDatos({periodoTributario: e.target.value})} placeholder="MM/AAAA (ej: 03/2026)" className="w-full md:w-64 p-3 border rounded-md shadow-sm outline-none focus:border-[#0071BC] text-black opacity-70" />
              </div>

              <div className="relative group">
                <label className="flex items-center text-gray-700 font-bold mb-2">
                  Tipo de declaración <span className="ml-2 text-blue-500 cursor-help text-xs">ⓘ</span>
                  <div className="absolute left-40 bottom-full mb-2 hidden group-hover:block w-48 p-2 bg-gray-800 text-white text-[10px] rounded shadow-lg z-50">Elija su tipo de declaración según:...</div>
                </label>
                <select value={datos.tipoDeclaracion} onChange={e => actualizarDatos({tipoDeclaracion: e.target.value})} className="w-full md:w-64 p-3 border rounded-md bg-white shadow-sm outline-none focus:border-[#0071BC] text-black opacity-60">
                  <option value="">Seleccione una opción</option>
                  <option value="Original">Original</option>
                  <option value="Sustitutoria">Sustitutoria</option>
              ...
              <div>
                ...
                <div className="space-y-3">
                  <label className="flex items-center space-x-3 cursor-pointer group">
                    <input type="radio" name="igv" checked={datos.condicionIgv === 'Cuenta propia'} onChange={() => actualizarDatos({condicionIgv: 'Cuenta propia'})} className="w-4 h-4 accent-[#0071BC] cursor-pointer" />
                    <span className="text-gray-600 group-hover:text-[#0071BC]">Cuenta propia</span>
                  </label>
                  <label className="flex items-center space-x-3 cursor-pointer group">
                    <input type="radio" name="igv" checked={datos.condicionIgv === 'Convenio de Estabilidad'} onChange={() => actualizarDatos({condicionIgv: 'Convenio de Estabilidad'})} className="w-4 h-4 accent-[#0071BC] cursor-pointer" />
            ...
            <div className="bg-gray-50 p-6 rounded-xl border border-gray-100 h-fit">
              ...
              <div className="space-y-6">
                {[
                  { label: "¿Ventas no gravadas (últimos 12 meses)?", state: datos.ventasNoGravadas, setter: (val: boolean) => actualizarDatos({ventasNoGravadas: val}) },
                  { label: "¿Impuesto al Arroz Pilado (IVAP)?", state: datos.ivap, setter: (val: boolean) => actualizarDatos({ivap: val}) }
                ].map((item, i) => (
                  <div key={i} className="flex justify-between items-center bg-white p-3 rounded-lg shadow-sm">
                    ...
                    <div className="flex items-center space-x-4">
                      ...
                      <div onClick={() => item.setter(!item.state)} className={`w-12 h-6 rounded-full p-1 cursor-pointer transition-colors duration-300 ${item.state ? 'bg-[#0071BC]' : 'bg-gray-300'}`}>
          ...
          <div className="mt-12 text-left">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
              {[
                ...
                <div key={i} onClick={() => actualizarDatos({regimenTributario: reg.n})} className={`bg-white border-2 hover:border-[#0071BC] p-4 rounded-xl shadow-sm cursor-pointer transition-all hover:shadow-lg group flex flex-col gap-9 h-29 ${datos.regimenTributario === reg.n ? 'border-[#0071BC]' : 'border-transparent'}`}>
            ...
            <div className="mt-6 w-full md:w-64">
              ...
              <select value={datos.otrosRegimenes} onChange={e => actualizarDatos({otrosRegimenes: e.target.value})} className="w-full p-2 border rounded-md bg-white shadow-sm text-sm outline-none focus:border-[#0071BC] text-gray-600">
                <option value="">Seleccione otros</option>
                <option value="Con convenio de Estabilidad">Con convenio de Estabilidad</option>
                <option value="Régimen Agrario – Ley 31110">Régimen Agrario – Ley 31110</option>
                <option value="Acuicultura – Ley 31666">Acuicultura – Ley 31666</option>
                <option value="Régimen Empresa Agraria">Régimen Empresa Agraria</option>
                <option value="Régimen Pequeño Agrario">Régimen Pequeño Agrario</option>
              </select>
            </div>
          </div>
