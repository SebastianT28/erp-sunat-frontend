...
export interface Bien {
  normalizado: boolean; codigo: string; descripcion: string
...
export default function EmisionGREBienes({ onVolver, onSiguiente }: EmisionGREBienesProps) {
  ...
  const puedeAgregarBien = modalCodigo.trim() !== "" && modalDescripcion.trim() !== "" && modalUnidad !== "" && modalPeso.trim() !== "" && modalCantidad.trim() !== ""

  const handleAgregarBien = () => {
    if (!puedeAgregarBien) return
    setBienes([...bienes, { normalizado: modalNormalizado, codigo: modalCodigo, descripcion: modalDescripcion, unidad: modalUnidad, peso: modalPeso, cantidad: modalCantidad }])
    handleCerrarModal()
  }

  const handleCerrarModal = () => { setModalAbierto(false); setModalNormalizado(false); setModalCodigo(""); setModalDescripcion(""); setModalUnidad(""); setModalPeso(""); setModalCantidad(""); setModalFrecuente(false) }
  const handleEliminarBien = (index: number) => { setBienes(bienes.filter((_, i) => i !== index)) }
  ...
  return (
    <div className="relative">
      ...
      <div className="mb-6 overflow-x-auto">
        <table className="w-full text-sm text-left border-b border-gray-200">
          <thead className="text-xs text-black font-extrabold border-b-2 border-gray-200 uppercase">
            <tr><th className="px-4 py-3">#</th><th className="px-4 py-3">Bien normalizado</th><th className="px-4 py-3">Código Bien</th><th className="px-4 py-3">Descripción</th><th className="px-4 py-3">Unid. medida</th><th className="px-4 py-3">Peso</th><th className="px-4 py-3">Cantidad</th><th className="px-4 py-3"></th></tr>
          </thead>
          <tbody>
            {bienes.length === 0 ? (
              <tr><td colSpan={8} className="px-4 py-6 text-center text-gray-500 font-medium">No hay registros disponibles</td></tr>
            ) : (
              bienes.map((b, i) => (
      ...
      <div className="flex gap-16 mb-8">
        <div><label className="text-black font-extrabold text-xs mb-2 block">Unidad de medida de peso bruto</label><select value={unidadPesoBruto} onChange={(e) => setUnidadPesoBruto(e.target.value)} className="border border-gray-300 px-3 py-2 text-sm text-black w-[250px] focus:outline-none focus:border-[#0063AE] bg-white">{unidadesMedidaPesoBruto.map((u) => (<option key={u} value={u}>{u}</option>))}</select></div>
        <div><label className="text-black font-extrabold text-xs mb-2 block">Peso bruto total de la carga</label><input type="text" value={pesoBrutoTotal} onChange={(e) => setPesoBrutoTotal(e.target.value)} className="border border-gray-300 px-3 py-2 text-sm text-black w-[250px] focus:outline-none focus:border-[#0063AE]" /></div>
      </div>
